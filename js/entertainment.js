const divMusic = document.querySelector("#mus_content")
...
function xhr_request(callback_functions = [], args = {}) {
    ...
    xhr.onload = () => {
        if (xhr.readyState == 4 && xhr.status == 200) {
            ...
            display_data(divMusic, items)
        }
        if (xhr.readyState == 4 && xhr.status > 400) {
            divMusic.innerHTML = `<h1>Error: ${xhr.status}</h1>
                    <p>${xhr.statusText}</p>`
        }
    }
    xhr.open("GET", "https://consequence.net/feed/")
    xhr.send()
...
function display_data(html_element, items) {
    ...
    for (let item of items) {
        ...
        <img src=${thumb} class='img-fluid rounded-start'>
        </div>
        <div class='col-md-8'>
        <div class='card-body'>
