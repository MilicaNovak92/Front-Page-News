...
function xhr_request(callback_functions=[], args = {}) {
    ...
    xhr.onload = () => {
        if (xhr.readyState == 4 && xhr.status == 200) {
            ...
            display_data(divContent, items)
        }
        if (xhr.readyState == 4 && xhr.status > 400) {
            divContent.innerHTML = `<h1>Error: ${xhr.status}</h1>
                    <p>${xhr.statusText}</p>`
        }
    }
    xhr.open("GET", "https://rss.nytimes.com/services/xml/rss/nyt/Technology.xml")
    xhr.send()
...
function display_data(html_element, items) {
    ...
    for(let item of items) {
        ...
        html_element.innerHTML += `
        <div class='card mb-3' style='max-width: 940px;'><div class='row g-0'><div class='col-md-4'><img src='${thumb}' style='margin-top: 1rem; width: 230px;' class='img-fluid rounded-start'>
